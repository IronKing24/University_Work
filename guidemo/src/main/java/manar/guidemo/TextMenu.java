...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; //controls text justifcation
	private JRadioButtonMenuItem center; //controls text justifcation
	private JRadioButtonMenuItem right; //controls text justifcation
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem linehight = new JMenuItem("Set Line Spacing...");
		linehight.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentHight = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "What font size do you want to use?",currentHight);
				if (s != null && s.trim().length() > 0) {
					try {
						double newhight = Double.parseDouble(s.trim()); // can throw NumberFormatException
						panel.getTextItem().setLineHeightMultiplier(newhight); // can throw IllegalArgumentException
						panel.repaint();
					}
					catch (Exception e) {
						JOptionPane.showMessageDialog(panel, s + " is not a legal text size.\n"
	...
	JMenu makeJustifyMenu ()
	{
		...
		center = new JRadioButtonMenuItem("Center");
